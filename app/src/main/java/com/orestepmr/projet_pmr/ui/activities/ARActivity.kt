...
class ARActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val modelUri = intent.getStringExtra("MODEL_URL") //On récupère depuis le QR code le nom de l'enigme
        Log.i("ARcheck", "onCreate: $modelUri")
        val key = extractKeyFromUrl(modelUri.toString())

        if (key == "chest.obj" || key == "cadena.obj" || key == "Cube.obj" || key == "marteau.glb"){ // Si c'est un modèle 3d
            load3DModel(key)
            Log.i("ARcheck", "onCreate: $key")
        } else if (key == "calcul"){ //Si c'est un calcul
            val resultat = calculEnigme() //On affiche et stock le resultat
        } else if (key =="devinette"){ //Si c'est une devinette
    ...
    private fun load3DModel(modelUri: String?) {
        ...
        val uri = Uri.parse("file:///android_asset/$modelUri") // Chemin correct pour le modèle dans les assets
        val renderableSource = RenderableSource.builder()
            .setSource(this, uri, RenderableSource.SourceType.GLB) // Utilisation de SourceType.OBJ pour un fichier OBJ
            .setRecenterMode(RenderableSource.RecenterMode.ROOT)
            .setScale(0.5f) // Réduisez la taille du modèle à la moitié de sa taille d'origine
            .build()
        ...
        ModelRenderable.builder()
            ...
            .exceptionally { throwable ->
                Log.e("ARActivity", "Unable to load model", throwable)
                null
            }
    }
